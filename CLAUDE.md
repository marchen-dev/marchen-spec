...
**CLI 命令**: `marchen` (已实现 info, init, new 命令)
...
- **apps/cli**: 命令注册（info, init, new）、参数解析、终端 UI（使用 commander + @clack/prompts）
- **packages/core**: 工作流用例（初始化、基础检查、变更创建）、领域规则、业务逻辑
- **packages/fs**: 文件系统操作（读写、YAML 处理）、路径解析、目录管理
- **packages/config**: 配置加载、默认值（specDirectory）、配置定义、模板管理
- **packages/shared**: 基础类型（PackageBoundary, ChangeMetadata, ArtifactDefinition）、常量（目录名）、错误定义（MarchenSpecError）
