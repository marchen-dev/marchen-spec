...
**CLI 命令**: `marchen`（已实现 init, new, list 命令）
...
- **apps/cli**: 命令注册（init, new, list）、参数解析、终端 UI（使用 commander + @clack/prompts）
- **packages/core**: 业务逻辑层，提供 `Workspace` 类（工作区上下文）和 `ChangeManager` 类（变更管理）
- **packages/fs**: 文件系统操作，拆分为 paths（路径解析）、directory（目录操作）、file（文件读写）、yaml（YAML 操作）四个模块
- **packages/config**: 配置加载、默认值（specDirectory）、Schema 定义、Artifact 模板管理
- **packages/shared**: 基础类型（PackageBoundary, ChangeMetadata, ArtifactDefinition）、常量（目录名）、错误定义（MarchenSpecError），拆分为 types / constants / errors 三个模块
...
# 运行单个测试文件
cd packages/core && pnpm test workspace.test.ts
```
...
### 已归档变更
- `2026-03-29-implement-fs-and-init-command` - 文件系统包和 init 命令实现
- `2026-03-29-implement-new-command` - new 命令实现
- `2026-03-30-implement-list-command` - list 命令实现
- `2026-03-30-refactor-to-class-architecture` - 重构为 Class 架构
...
- CLI 二进制名称: `marchen`（在 package.json 的 bin 字段定义）
...
2. **CLI 职责**: CLI 层只负责用户交互，业务逻辑通过 core 包的 Class 实例调用
...
7. **Class 架构**: core 包使用 `Workspace` + `ChangeManager` 类组织业务逻辑，CLI 创建实例后调用方法
