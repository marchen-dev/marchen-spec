...
### 需求: rapid schema SHALL 只包含 tasks 一个 artifact

rapid schema 的 artifacts 数组 MUST 只有一个元素：tasks，且 requires 为空数组。
...
### 需求: rapid 的 tasks 模板 SHALL 包含背景章节

tasks.md 模板 MUST 在任务列表之前包含 `## 背景` 章节，供用户填写变更目的和方案。

#### 场景: 查看 rapid 变更的 tasks 模板

WHEN 创建 rapid 变更后查看 tasks.md
THEN 文件包含 `## 背景` 章节（带注释提示）和任务 checkbox 骨架

### 需求: rapid 的 tasks instruction SHALL 不引用 specs 和 design

instruction 文本 MUST 不提及 specs、design、capability 等 spec-driven 特有概念。

#### 场景: 获取 rapid tasks 的 instruction
