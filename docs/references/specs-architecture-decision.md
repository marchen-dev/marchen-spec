...
MarchenSpec 只有一个固定工作流（full: proposal → specs → design → tasks），为 4 节点 DAG 写通用图引擎是过度设计。依赖关系直接硬编码在 ChangeManager 中。
...
| 多 schema | 支持（spec-driven, task-only, 自定义） | 支持（full, lite） |
